...
将上述解压后的目录配置到环境变量 VISUALIS_HOME

```bash
export VISUALIS_HOME=~/app/davinci/davinci-assembly_3.0.1-0.3.0-SNAPSHOT-dist
```
...
```bash
mysql -P 3306 -h localhost -u root -proot davinci0.3 < $VISUALIS_HOME/bin/davinci.sql
```
